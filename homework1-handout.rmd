...
```{r}
wage.obs <- c(1:3000)       
wages.cut <- cut(wage.obs, 5, label = FALSE) %>% 
  sample()

age_predictors <- poly(Wage$age, degree=10, raw=TRUE)

age.pred.df <- as.data.frame(age_predictors) %>% 
    cbind(wage = Wage$wage)

```

```{r}
do.wage <- function(chunkid, chunkdef, dat){  # function argument
...
```{r}
error.folds.wages = NULL

tmp.wage = ldply(1:nfold, do.wage,
    chunkdef=wages.cut, dat=age.pred.df)
  
error.folds.wages=rbind(error.folds.wages, tmp.wage)

wage.error.Melted <- reshape2::melt(error.folds.wages, id.var='fold')

wage.error.plot <- ggplot() +
  geom_line(data = wage.error.Melted, aes(x = fold, y = value, col = variable)) 
