...
# this doesn't work feel free to change any of this code chunk
do.poly <- function(chunkid, chunkdef, dat, p){  # function argument
    
    train = (chunkdef != chunkid)
    
    predictors <- poly(dat, degree=p, raw=TRUE)
    
    Xtr = predictors[train,1:(p-1)]  # get training set
    Ytr = predictors[train,p]  # get true response values in trainig set

    Xvl = predictors[!train,1:(p-1)]  # get validation set
    Yvl = predictors[!train,p]  # get true response values in validation set

    poly.wage <- lm(wage ~ predictors, data = Wage)
    
    predYtr = predict(poly.wage)  # predict training values
    predYvl = predict(poly.wage,Xvl)  # predict validation values
...
for (j in allP){
  ...
  tmp.wage = ldply(1:nfold, do.poly,
    chunkdef=wage.obs, dat=Wage$age, p=j)
  
  error.folds=rbind(error.folds.wages, tmp.wage)
