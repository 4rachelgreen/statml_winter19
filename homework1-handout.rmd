...
cv.error.5 <- as.data.frame(cv.error.5)
```


```{r}
do.wage <- function(chunkid, chunkdef, p){  # function argument
    
    train = (chunkdef != chunkid)
    if (p == 1){

      predictors.df <- as.data.frame(cbind("1" = Wage$age, wage = Wage$wage))
      
      Xtr = as.data.frame(predictors.df[train,1])  # get training set
      Ytr = predictors.df[train,2]  # get true response values in trainig set
  
      Xvl = as.data.frame(predictors.df[!train,1])  # get validation set
      Yvl = predictors.df[!train,2]  # get true response values in validation set
      
      lm.wage <- lm(wage~., data = predictors.df[train,1:2])
      ...
      cbind(wage = Wage$wage)
    
      predictors.df <- as.data.frame(predictors)
    
      Xtr = predictors.df[train,1:p]  # get training set
      Ytr = predictors.df[train,p+1]  # get true response values in trainig set

      Xvl = as.data.frame(predictors.df[!train,1:p])  # get validation set
      Yvl = predictors.df[!train,p+1]  # get true response values in validation set
      
      lm.wage <- lm(wage~., data = predictors.df[train,1:ncol(predictors.df)])
...
```{r}
predictors <- poly(Wage$age, 10, raw = TRUE) %>% 
      cbind(wage = Wage$wage)
...
do.wage <- function(chunkid, chunkdef, p){  # function argument
      ...
      Xtr = as.data.frame(predictors.df[train,1:p])  # get training set
      Ytr = predictors.df[train,11]  # get true response values in trainig set

      Xvl = as.data.frame(predictors.df[!train,1:p])  # get validation set
      Yvl = predictors.df[!train,11]  # get true response values in validation set
      
      lm.wage <- lm(wage~., data = predictors.df[train,1:p])
...
error.wages.melt = melt(error.folds.wages, id=c("degree","fold")) %>% 
  group_by(degree)

wage.error.plot <- ggplot() +
  geom_line(data = error.wages.melt, aes(x = fold, y = value, col = variable)) +
  facet_wrap(~degree)
